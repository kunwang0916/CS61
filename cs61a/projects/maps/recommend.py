...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    return group_by_first([[find_closest(restaurant_location(res), centroids), res] for res in restaurants])
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        glusters = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(g) for g in glusters]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxy = sum([(xi - meanx)*(yi - meany) for xi, yi in zip(xs, ys)])
    b = sxy / sxx
    a = meany - b * meanx
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    best_fn = max(feature_fns, key=lambda f: find_predictor(user, reviewed, f)[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for res in restaurants:
        res_name = restaurant_name(res)
        if res in reviewed:
            results[res_name] = user_rating(user, res_name)
        else:
            results[res_name]= predictor(res)
    return results
    # END Question 9

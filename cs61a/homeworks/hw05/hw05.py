...
def balanced(m):
    ...
    rs = right(m)
    rse = end(rs)
    if total_weight(lse) * length(ls) != total_weight(rse) * length(rs):
        return False
    if is_mobile(lse) and not balanced(lse):
        return False
    if is_mobile(rse) and not balanced(rse):
        return False
    return True
...
def totals_tree(m):
    ...
    lse = end(left(m))
    if is_mobile(lse):
        branches.append(totals_tree(lse))
    else:
        branches.append(tree(total_weight(lse)))

    rse = end(right(m))
    if is_mobile(rse):
        branches.append(totals_tree(rse))
    else:
        branches.append(tree(total_weight(rse)))
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attempts = []
    account_info = [balance, password]
    def w(amount, password):
        if len(attempts) == 3:
            return "Your account is locked. Attempts: " + str(attempts)
        if password != account_info[1]:
            attempts.append(password)
            return "Incorrect password"
        if amount > account_info[0]:
            return "Insufficient funds"
        account_info[0] = account_info[0] - amount
...
def generate_paths(t, x):
    ...
    result = []
    for b in branches(t):
        sub_path = generate_paths(b, x)
        for sp in sub_path:
            result.append([label(t)] + sp)
    return result

...
public class SeparableEnemySolver {
    ...
    public boolean isSeparable() {
        ...
        for (String l: g.labels()) {
            if (groupA.contains(l) == false && groupB.contains(l) == false) {
                groupA.add(l);
            }
            for (String n: g.neighbors(l)) {
                if (groupA.contains(l)) {
                    if (groupA.contains(n)) {
                        return false;
                    }
                    groupB.add(n);
                } else if (groupB.contains(l)) {
                    if (groupB.contains(n)) {
                        return false;
                    }
                    groupA.add(n);
                }
